...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # to the layer output.
        ##################################################################
        x_upsample_res = self.upsample_residual(x)
        x = self.layers(x)

        return x + x_upsample_res
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        x_downsample_res = self.downsample_residual(x)
        x = self.layers(x)
        return x + x_downsample_res
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4):
        ...
        # what is number of input channels into first resblock up
        # we b,2048 after dense
        # b, c, 4, 4
        # c = 2048/16
        self.layers = nn.Sequential(
            # ResBlockUp(input_channels=int(2048/(starting_image_size**2))),
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        # z is b, 128 (sampled noise)
        x = self.dense(z)
        # x is just b,2048
        # need to reshape into image 
        b = z.shape[0]
        # x = x.view(b, -1, self.starting_image_size, self.starting_image_size)
        x = x.view(b, -1, 4, 4)
        x = self.layers(x)
        return x
        ##################################################################
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # x is b,3,h,w
        x = self.layers(x)
        # now x is b,128, h,w where h,w downsampled 
        # need to sum across img dims
        x = torch.sum(x, dim=(2, 3))
        # now we should have b,128
        x = self.dense(x)
        return x
